 ...
 * @date 2018年11月8日上午11:19:16
 */
public class ConeHeadZombie extends Zombie implements WalkingDead{
	protected static Image cardImage;
	protected static Image descirptionImage;
	private static Map<String, Image> images;//键值对保存各种图片
	
	static {
		cardImage=new ImageIcon(SystemConstant.ZOMBIE_CARD_PATH+"Conehead ZombieCard.png").getImage();
		descirptionImage=new ImageIcon(ZOMBIE_DESCRIPTION_PATH+"ConeheadZombieAbout.png").getImage();
		images = new HashMap<>();
		...
		images.put("eatingImage", new ImageIcon(SystemConstant.ZOMBIES_PATH+"ConeheadZombie/ConeheadZombieAttack.gif").getImage());
		images.put("lostOrnamentEating", new ImageIcon(SystemConstant.ZOMBIES_PATH+"Zombie/ZombieAttack.gif").getImage());
	}

	public ConeHeadZombie(){
		this.activeImage=new ImageIcon(ZOMBIES_PATH+"ConeheadZombie/ConeheadZombie.gif").getImage();
	}
	
	public <T extends Controller>ConeHeadZombie(int movingRow, T pc){
		super(movingRow, pc);
		this.col_posX = posX + 70;
        this.col_posY = posY + 50;
        this.col_width = 40;
        this.col_height = 80;
		this.current_health = 640;//路标头370+本体200+临界点70
		changeSpeed(4700);
		this.damage = 100/pc.getFPS();//每秒100除以帧数,每帧的伤害(持续)
	...
	public ConeHeadZombie(int movingRow, AdventureController pc){
		super(movingRow, pc);
		this.col_posX = posX + 70;
        this.col_posY = posY + 50;
        this.col_width = 40;
	...
	public int criticalPoint() {
		//临界点 270/70/0,掉路障/掉头/死亡
		if(this.current_health <= 640 && this.current_health > 270) {
	...
	public Image getImageByName(String image_name) {
		if(image_name.equals("boomDieImage"))
			...
			return Toolkit.getDefaultToolkit().createImage(ZOMBIES_PATH+"ConeheadZombie/ConeheadZombie.gif");
